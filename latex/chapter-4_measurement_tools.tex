...
%
    ...
    \subsection{Implementation}
        ...
        This helps to reduce processing overhead, and filling up the queue with data, that does not matter to the collection.
        ...
        As the original messages has been encoded base 16 before transmission, the first step is to decode the string. The resulting base 64 encoded string is also decoded and stored as raw bytes in a file. These raw bytes are decrypted with \textit{Openssl} using the matching private key. The servers simplified overview process overview is shown in figure \ref{fig:server_proc}.\\
