...
\label{sec:software_design:data_collection}
 ...
 %
    ...
    \subsection{ID generation}
        ...
        To further strengthen the ID against brute forcing and decreasing the amount of character used in a DNS query, the generated ID should be reduced to feasible number of bytes.\\
...
%
    While the ID is generated in a way to transmitted over DNS, the collected data is still in plain text and a large chunk of text. As we have shown in \ref{subsec:related:dns} only 63 byte long labels up to a total of 255 bytes for a complete request are supported.
