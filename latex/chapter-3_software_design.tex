...
%
    ...
    \subsection{Fitting data to DNS request}
        ...
        As elements from the domain name system are required to support [1-z], [0=9] and hyphens only the data needs to be transformed to a format, that can be transmitted reliable.
        Therefore a base32 encoding would be feasible to keep the transmission size as low as possible. As base32 uses the equal sign as padding char, it would be required to exchange it with a hyphen, to make it conforming. 
\newpage
